<div class="col-lg-6 admin-branches" x-data="adminBranches">
    @forelse($branches as $branch)
        <div class="card mb-3 branch">
            <div class="card-header d-flex justify-content-between align-items-center cursor-pointer"
                x-bind:class="[ {{ $branch->branch_id }} === {{ $activeBranchId }} ? 'text-white bg-primary' : 'text-dark']"
                title="Click to Activate." wire:click="changeBranch({{ $branch->branch_id }})">
                <span class="d-inline-block text-bold"> &#62;</span>

                <div class="w-10 d-flex">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20.51" height="13.821" viewBox="0 0 20.51 13.821"
                        fill="{{ $branch->branch_id === $activeBranchId ? 'white' : '#463291' }}">
                        @if($branch->branch_name === 'F&F.Co Main Office')
                            <path class="a" d="M-4018.3-2526.078l1.835,1.063,3.6-2.078v-2.125Z" transform="translate(4024.577 2533.469)" />
                            <path class="a" d="M-4415.4-2806.07v2.138l-8.035,4.646,7.307,4.213-1.843,1.067-9.138-5.28Z" transform="translate(4427.111 2806.07)" />
                            <path class="a" d="M-3886.51-2056.113l-3.137,1.812v1.061l4.053-2.342-.916-.53m0-.532,1.835,1.063-5.434,3.139v-2.125Z" transform="translate(3898.353 2068.151)" />
                            <path class="a" d="M-3889.54-2280.515l10.329-5.984-8.218-4.749-.922.534,6.616,3.815.691.4-.691.4-7.805,4.512v1.073m-.46.8v-2.138l8.035-4.646-7.307-4.213,1.843-1.067,9.138,5.281Z" transform="translate(3898.247 2299.676)" />
                        @elseif($branch->branch_type === 'salon')
                            <path
                                d="M-2574.614-2765.961c-.336.061-.654.106-.946.136a6.74,6.74,0,0,1-.933.036,4.36,4.36,0,0,1-.937-.141,4.1,4.1,0,0,1-.7-.254,3.453,3.453,0,0,1-.433-.244q-.077-.051-.149-.108l14.8-4.112-.126-.472,1.78-.477.119.445,1.187-.318a1.229,1.229,0,0,0,.869-1.5,1.229,1.229,0,0,0-1.5-.869,1.229,1.229,0,0,0-.869,1.5l-1.78.477a3.1,3.1,0,0,1-.1-.614,3.069,3.069,0,0,1,1.69-2.927,3.1,3.1,0,0,1,1.2-.321,3.07,3.07,0,0,1,2.927,1.69,3.1,3.1,0,0,1,.32,1.194,3.07,3.07,0,0,1-1.678,2.923,3.1,3.1,0,0,1-.578.222l-12.646,3.389-.146.04C-2573.5-2766.2-2574.016-2766.069-2574.614-2765.961Z"
                                transform="translate(2578.706 2775.729)" />
                            <path
                                d="M-2573.373-2522.357c-.245-.065-.743-.191-1.325-.3-.325-.059-.631-.1-.91-.131a6.278,6.278,0,0,0-.868-.034,3.879,3.879,0,0,0-.837.126l-.126.036,13.649,3.793.44.122-.118.441-.007.028.89.238.119-.445.445.119,1.187.318a1.689,1.689,0,0,1,1.194,2.069,1.689,1.689,0,0,1-2.069,1.194,1.689,1.689,0,0,1-1.252-1.607l-.912-.245c0,.022,0,.045-.005.067a2.6,2.6,0,0,0,.138,1,2.618,2.618,0,0,0,.865,1.211,2.6,2.6,0,0,0,.434.273,2.641,2.641,0,0,0,1.017.272,2.6,2.6,0,0,0,1-.138,2.618,2.618,0,0,0,1.211-.865,2.578,2.578,0,0,0,.273-.433,2.65,2.65,0,0,0,.272-1.016,2.594,2.594,0,0,0-.136-1,2.619,2.619,0,0,0-.859-1.21,2.581,2.581,0,0,0-.431-.274,2.646,2.646,0,0,0-.49-.188l-12.655-3.391-.137-.038m.119-.445.147.041,12.646,3.388a3.094,3.094,0,0,1,.579.222,3.059,3.059,0,0,1,.935.732,3.076,3.076,0,0,1,.72,1.59,3.04,3.04,0,0,1,.023.6,3.106,3.106,0,0,1-.32,1.194,3.056,3.056,0,0,1-.731.941,3.079,3.079,0,0,1-1.593.725,3.058,3.058,0,0,1-.6.024,3.1,3.1,0,0,1-1.2-.32,3.057,3.057,0,0,1-.941-.731,3.081,3.081,0,0,1-.725-1.593,3.061,3.061,0,0,1-.024-.6,3.093,3.093,0,0,1,.1-.614l1.78.477a1.229,1.229,0,0,0,.869,1.5,1.229,1.229,0,0,0,1.5-.869,1.228,1.228,0,0,0-.869-1.5h0l-1.187-.318-.119.445-1.78-.477.127-.473-14.8-4.111a1.844,1.844,0,0,1,.149-.109,3.408,3.408,0,0,1,.433-.243,4.038,4.038,0,0,1,.7-.254,4.323,4.323,0,0,1,.937-.141,6.71,6.71,0,0,1,.933.036c.291.03.609.075.945.135.6.108,1.11.238,1.362.305Z"
                                transform="translate(2578.708 2527.159)" />
                        @elseif($branch->branch_type === 'gym')
                            <path d="M-3263.553-2367.89h2l-3.241,14.05h-2Z" transform="translate(3266.794 2367.89)" />
                            <path d="M-3011.3-2097.376h-.446l1.283-5.529h2l-.855,3.686h7.235l-.43,1.843Z"
                                transform="translate(3015.663 2106.974)" />
                            <path
                                d="M-2507.1-1969.236l3.028-13.128h-1.051l-3.029,13.128h1.051m.366.461h-2l3.241-14.05h2Z"
                                transform="translate(2520.372 1988.738)" />
                            <path
                                d="M-3050.934-1692.429l-.215.921h7.235l-.131.565-.724,3.121h1.051l1.069-4.607h-8.284m-.365-.461h9.229l-1.283,5.528h-2l.855-3.686h-7.235Z"
                                transform="translate(3055.032 1703.255)" />
                        @elseif($branch->branch_type === 'spa')
                            <path
                                d="M-2840.731-3120.9h0l-4.56-4.556a7.844,7.844,0,0,1-.527-.583,7.665,7.665,0,0,1-.457-.621c-.139-.21-.269-.431-.387-.654s-.222-.45-.316-.681-.176-.465-.246-.7-.129-.478-.176-.719-.082-.486-.105-.73-.035-.491-.035-.735.012-.492.035-.736.059-.49.105-.73.106-.483.176-.719.153-.474.246-.7.2-.46.316-.681.248-.443.387-.654.294-.421.457-.621a7.837,7.837,0,0,1,.527-.583l4.561-4.557.016.016h-.016v2.6l-3.035,3.033a6.244,6.244,0,0,0-.422.466,6.126,6.126,0,0,0,0,7.748c.114.14.236.276.362.406l3.094-3.086v2.6l-1.792,1.787,1.793,1.792v2.6Z"
                                transform="translate(2847.54 3140.86)" />
                            <path
                                d="M-2352.631-3139.754v1.311l2.9,2.9a6.665,6.665,0,0,1,.453.5,6.511,6.511,0,0,1,.393.534,6.559,6.559,0,0,1,.6,1.148,6.592,6.592,0,0,1,.363,1.223,6.637,6.637,0,0,1,.121,1.26,6.639,6.639,0,0,1-.072.977l-.135.9-.646-.646-3.979-3.976v1.3l3.987,3.984.241.24-.159.3c-.1.187-.208.37-.324.546s-.253.363-.393.534-.294.341-.453.5l-2.9,2.9v1.3l3.774-3.771a7.315,7.315,0,0,0,.5-.548,7.2,7.2,0,0,0,.429-.583c.131-.2.253-.4.363-.614s.209-.422.3-.64.165-.437.231-.66.121-.449.165-.676.077-.457.1-.687.033-.461.033-.691-.011-.462-.033-.691-.055-.46-.1-.686-.1-.455-.165-.676-.144-.445-.231-.66-.188-.433-.3-.64-.232-.416-.363-.614a7.138,7.138,0,0,0-.429-.583,7.315,7.315,0,0,0-.5-.548l-3.774-3.771m-.461-1.112h0l4.56,4.556a7.786,7.786,0,0,1,.527.583q.244.3.457.621c.139.211.269.431.386.654s.223.45.316.681.176.466.246.7.129.478.176.719.082.486.105.73.035.491.035.735-.012.492-.035.735-.059.49-.105.73-.106.483-.176.719-.153.474-.246.7-.2.46-.316.681-.247.443-.386.654-.294.422-.457.621a7.787,7.787,0,0,1-.527.583l-4.56,4.557v-2.6l3.035-3.033a6.251,6.251,0,0,0,.422-.466,6.151,6.151,0,0,0,.365-.5q.163-.246.3-.507l-4.122-4.119v-2.6l4.766,4.762a6.223,6.223,0,0,0,.067-.909,6.17,6.17,0,0,0-.112-1.171,6.148,6.148,0,0,0-.338-1.138,6.1,6.1,0,0,0-.562-1.067,6.108,6.108,0,0,0-.365-.5,6.207,6.207,0,0,0-.422-.466l-3.035-3.033Z"
                                transform="translate(2360.684 3140.866)" />
                        @endif
                    </svg>
                </div>

                <div class="w-90 d-flex justify-content-between">
                    <div>
                        <strong class="d-block mb-0">{{ $branch->branch_name }}</strong>
                        <span class="mb-0">Branch ID: {{ strtoupper($branch->branch_code) }}</span>
                    </div>

                    <div>
                        <p class="mb-0">Status: {{ $branch->branch_status }}</p>
                        <p class="mb-0">Date Opened:
                            {{ date('M. d, Y', strtotime( $branch->created_at) ) }}
                        </p>
                    </div>
                </div>
            </div>
            {{-- ./card-header --}}
            <div class="card-body branch__info">
                <div class="d-flex justify-content-around">
                    <div class="w-12 d-flex justify-content-center">
                        <span class="icon icon__location--black mr-0"></span>
                    </div>
                    <div class="w-75">
                        <address class="mb-4">{{ $branch->branch_address }}</address>
                    </div>
                    <div class="w-25"></div>
                </div>

                @forelse($branch->users as $user)
                    <div class="d-flex branch__users" x-data="{ userStatus: '{{ $user->user_status }}' }"
                        :class="{ 'opacity-point5' : userStatus === 'inactive' }">
                        <div class="w-15 d-flex justify-content-center">
                            <span class="icon icon__account--black mr-0"></span>
                        </div>

                        <div class="user_info w-75">
                            <p class="mb-0">
                                <b>Name:</b>
                                {{ sprintf('%s %s', $user->first_name, $user->last_name) }}
                            </p>
                            <p class="mb-0"><b>Type:</b> {{ $user->role->name }}</p>
                            <p class="mb-0"><b>Email:</b> {{ $user->email }}</p>
                        </div>

                        <div class="switcher">
                            <label class="switch">
                                <input type="checkbox" wire:click="toggleUserStatus({{ $user->user_id }})"
                                    x-bind:checked="userStatus === 'active'" />
                                <span class="slider round"></span>
                            </label>
                        </div>
                    </div>
                    <br />
                @empty
                    <h6>No User yet..</h6>
                @endforelse

            </div>
            {{-- ./ card-body --}}
        </div>
    @empty
        <h2>Empty Branch...</h2>
    @endforelse
</div>

<script>
    function adminBranches() {
        return {

        };
    }

</script>
